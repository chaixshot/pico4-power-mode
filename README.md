...
- 下拉框选项与按钮文字统一显示为「**性能**」而非原厂文案「效果优先」。
...
4. 重启设置 app（`pkill -f com.picovr.settings`），打开「设置 → 实验室 → 电源管理方案」，即可看到并选择「性能」。
...
2. **`PopupMenuHelper.c(...)`** —— 反射往电源菜单的 `List<MenuItemData>` 追加第三个「性能」项（文本直接用 `MenuItemData.l(CharSequence)` 设置为「性能」）。
