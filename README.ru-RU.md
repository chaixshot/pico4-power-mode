...
- **Двунаправленное принуждение FFR** (`persist.pvr.config.ffr`):
  - Производительность (2) → **выкл** (`0`, без размытых краёв, вся производительность GPU идёт на частоту кадров)
  - Качество (3) / Стандарт / Экономия (0/1) → **вкл** (`1`, системное значение по умолчанию; для 2448 в режиме качества FFR нужен для стабильной частоты кадров)
