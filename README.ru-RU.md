...
3. **`U0(int)`** — перехватывает `i==2`, вызывает `DeviceSwitchUtilsKt.e(context, 2)` (системное переключение, пишет `powerlevel=2` и persist props), обновляет текст кнопки.
...
- В прошивке `PXRuleValueFile.txt` для высокопроизводительного режима `eyebuffer=1504`, но системное переключение (`DeviceSwitchUtils.e`) принудительно выставляет **2048** — чётче, но сильнее грузит GPU.
