...
- И пункт меню, и текст кнопки показывают «**性能**» вместо стокового «效果优先».
...
4. Перезапустите приложение настроек (`pkill -f com.picovr.settings`), откройте `Настройки → Лаборатория → Управление питанием` и выберите «性能».
...
2. **`PopupMenuHelper.c(...)`** — отражательно добавляет в `List<MenuItemData>` меню третий пункт «性能» (текст через `MenuItemData.l(CharSequence)`).
...
4. **`Q(int)`** — показывает «性能» в кнопке/текущем режиме.
